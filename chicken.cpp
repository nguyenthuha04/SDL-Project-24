...
void Chicken:: InitBullet(bullet* p_bullet, SDL_Renderer*screen){
 if(p_bullet!=NULL){
    if(p_bullet->loadImg("egg.png", screen)){
        p_bullet->set_is_move(true);
        p_bullet->SetRect(this->rect_.x+WIDTH_CHICKEN/2-p_bullet->GetRect().w/2,this->rect_.y+HEIGHT_CHICKEN);
...
void Chicken:: Move(){
 if(come_back_==true){
    rect_.y+=y_val_;
 if(status_.turn_left){ rect_.x-=x_val_;}
    if(status_.turn_right){ rect_.x+=x_val_;}
    if(status_.turn_up){ rect_.y-=2*y_val_;}
    ...
    if(rect_.x<=-WIDTH_CHICKEN*2){
        status_.turn_right=1;
        status_.turn_left=0;
    }
    else if(rect_.x>= SCREEN_WIDTH+2*WIDTH_CHICKEN){
        status_.turn_left=1;

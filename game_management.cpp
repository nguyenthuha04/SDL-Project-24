...
game_management:: game_management(){
 ...
 if(SDL_Init(SDL_INIT_VIDEO)<0){
    ...
    g_font_text=TTF_OpenFont("font1.ttf",24);
 ...
 void game_management:: menu(const std:: string& item){
  ...
  while(quit){
    ...
    while(SDL_PollEvent(&gEvent)!=0){
        plane1.Control(gEvent,gRenderer/*,bullet_level*/);
...
void game_management::handle_boss(){
   //handle boss
   if(kill>=NUMBER_OF_CHICKEN && Boss.get_heart()>=0){
    ...
    for(int sb=0;sb<s_bullet_list.size();sb++){
        ...
        if(p_bullet!=NULL){
            ...
            if(Col3){
                Boss.Decrease((plane1.get_bullet_damage())/*+bullet_level*BULLET_DAMAGE_LEVEL_UP*/);
 ...
 if(kill<NUMBER_OF_CHICKEN*2){
    ...
    for(int sb=0;sb<s_bullet_list.size();sb++){
        ...
        if(p_bullet!=NULL){
            ...
            if(Col3){
                p_chicken->Decrease((plane1.get_bullet_damage())/*+bullet_level*BULLET_DAMAGE_LEVEL_UP*/);
                plane1.RemoveBullet(sb);
...
void game_management:: handle_game(){
  ...
  /* gift.Move(SCREEN_WIDTH,SCREEN_HEIGHT);
   gift.Show(gRenderer);
   bool Col4=check_collision(plane1.GetRect(),gift.get_gift_type()<LEVEL_UP ? gift.GetRect(): gift.GetRectFrame());
   if(Col4){
    gift.SetRect(-GIFT_WIDTH,-GIFT_HEIGHT);
    if(bullet_level<3 && ((gift.get_gift_type()==plane1.get_bul_type())|| gift.get_gift_type()==LEVEL_UP)){
        bullet_level++;
    }
    if(gift.get_gift_type()<LEVEL_UP){
        plane1.set_bul_type(gift.get_gift_type());
    }
